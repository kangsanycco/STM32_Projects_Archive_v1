...
void BSP_Servo_SetSpeed(uint8_t ch, uint8_t speed) {   // 어떤 채널을 움직일 지는 더 상위 계층에서 진행
    ...
    if (HAL_I2C_Mem_Write(I2C_MOTOR_DRV, ADDR_I2C_PCA9685, reg_base, I2C_MEMADD_SIZE_8BIT, data, 4, 100) != HAL_OK) {

        // 여기에 브레이크 포인트를 걸어보세요.
        // 만약 HAL_ERROR나 HAL_BUSY가 뜬다면 하드웨어 연결(선 빠짐) 문제나 전원 문제입니다.
    }
}
