 ...
 */
typedef enum {
    STATE_CONVEY_PAUSED_ROBOT_BUSY = 0,     // 컨베이어 중지, 로봇 작동 중
 ...
 */
typedef enum {
	STATE_LIFT_INITIALIZING = 1,			// 전원이 켜지면, 리프트가 원점을 잡는 과정
    STATE_LIFT_MOVING = 2,        		    // 목표 층으로 리니어 모터 이동 중
    STATE_RACK_INSERTING = 3,      		    // 목표 층 도착 후 컨베이어 가동, 랙에 투입
    STATE_LIFT_RETURNING = 4,      		    // 1층으로 복귀 중
} LiftRackState_t;
...
typedef struct {
    // 1. 시스템 제어 및 모니터링
	MainControlState_t mainState;    // <--- 현재 메인 상태(STATE_BOOT, STATE_IDLE, STATE_RUNNING 등) 저장
    ...
    // 3. 물리 센서 실시간 상태 (config.h와 1:1 매칭)
